...
from cords.utils.data.datasets.SL.custom_dataset_selcon import CustomDataset_SELCON, CustomDataset_WithId_SELCON, \
...
class SELCONstrategy(DataSelectionStrategy):
    ...
    def __precompute(self, f_pi_epoch, p_epoch, alphas):  # TODO: alphas?
        ...
        for _, (inputs, targets, idxs) in enumerate(loader_tr):
            ...
            for i in range(p_epoch):
                ...
                step_size = self.lr * math.sqrt(1. - bias_correction2) / (
                            1. - bias_correction1)  # doubt: why sqrt only on numerator?
            ...
            for batch_idx_val, (inputs_val, targets_val, _) in enumerate(loader_val):
                ...
                exten_val = torch.cat((inputs_val, torch.ones(inputs_val.shape[0], device=self.device).view(-1, 1)),
                                      dim=1)
    ...
    def __return_subset(self, theta_init, p_epoch, curr_subset, budget,
        ...
        loader_tr = torch.utils.data.DataLoader(SubsetDataset_WithId_SELCON(self.trainset, curr_subset), shuffle=False,
                                                batch_size=batch)
        # loader_tr = self.trainloader

        sum_error = torch.nn.MSELoss(
            reduction='sum')  # doubt: why not use self.criterion here, also check the reduction here and nored
        ...
        for batch_idx, (inputs, targets, _) in enumerate(loader_tr):
            ...
            for _ in range(p_epoch):
                ...
                for batch_idx_trn, (inputs_trn, targets_trn, _) in enumerate(loader_tr):
                    ...
                    exten_trn = torch.cat((inputs_trn, torch.ones(inputs_trn.shape[0] \
                                                                  , device=self.device).view(-1, 1)), dim=1).to(
                        device_new)
                ...
                weight_grad = fin_trn_loss_g + 2 * rem_len * \
                              torch.cat((weights[:, :-1], torch.zeros((weights.shape[0], 1), device=self.device)),
                                        dim=1)  # +\
            ...
            for batch_idx_trn, (inputs_trn, targets_trn, _) in enumerate(loader_tr):
                ...
                exten_trn = torch.cat((inputs_trn, torch.ones(inputs_trn.shape[0], device=self.device).view(-1, 1)),
                                      dim=1)
